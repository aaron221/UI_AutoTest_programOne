 ...
 */
public class WeixinMovParkManage {
    ...
    @Test
    public void weixinMovParkManage() {
        Boolean flag = true;
        try {
        ...
        try {
            for (int i = 1; i <= 2; i++) {
                if (i == 1) {
                    ...
                    driver.findElement(By.xpath("//a[text()='订阅号']")).click();//点击订阅号页签按钮
                    Thread.sleep(1000);
                    driver.findElement(By.id("authenticationType3")).click();//是否认证-是
                    driver.findElement(By.id("dimName")).sendKeys("公司测试机微信" + i);
                    driver.findElement(By.id("dimNumber")).sendKeys("gh_31a2a1e1641f");
                    driver.findElement(By.id("dappID")).sendKeys("wxd8e0da0e36564d5f");
                    driver.findElement(By.id("dappKey")).sendKeys("8fda39e7daa24579d5765f1430442ab3");
                    driver.findElement(By.id("editBtn_2")).click();//点击保存按钮
                    driver.findElement(By.xpath("//button[@id='editBtn_2']/following-sibling::button")).click();//点击取消按钮退出新增页面
                    Thread.sleep(1000);
                } else {
                    Log.startTestCase("修改微信订阅号用例");
                    ...
                    driver.findElement(By.xpath("//button[@id='editBtn']/following-sibling::button")).click();//点击取消按钮退出新增页面
                    Thread.sleep(1000);
                }
                //判断是否新增修改成功
                ...
                if (weixinName.equals("公司测试机微信1")) {
                    Log.info("新增微信订阅号成功");
                    Log.endTestCase("新增微信订阅号用例");
                } else if (weixinName.equals("公司测试机微信2")) {
                    Log.info("修改微信订阅号成功");
                    Log.endTestCase("修改微信订阅号用例");
                } else {
                    Log.error("新增或修改微信订阅号失败");
                    flag = false;
                }
            }
        } catch (InterruptedException e) {
            new GetScreenshot(driver, "新增或修改微信订阅号过程中出现了错误");
            Log.error("新增或修改微信服订阅号过程中出现了错误");
            e.printStackTrace();
            Assert.fail();
        }

        Log.startTestCase("删除微信订阅号用例");
        try {
            DriverWait.findElement(driver, driver.findElement(By.xpath("//div[@id=\"accessPane\"]/div[2]" +
            ...
            new GetScreenshot(driver, "删除微信订阅号过程中发生了错误");
            Log.error("删除微信订阅号过程中发生了错误");
            e.printStackTrace();
            Assert.fail();
        }
        Log.endTestCase("删除微信订阅号用例");
